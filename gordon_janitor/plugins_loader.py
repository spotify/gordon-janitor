...
``setup.py``, registered under ``'gordon_janitor.plugins'``. For
example:
    ...
    setup(
        ...
        entry_points={
            'gordon_janitor.plugins': [
                'gcp.gdns = gordon_janitor_gcp.gdns:DNSProviderClient',
            ],
        },
        # snip
    )

Plugins are initialized with any config defined in
``gordon-janitor-user.toml`` and ``gordon-janitor.toml``. See
...
example, ``gcp.gpubsub`` will not have access to the configuration
for ``gcp.gdns``.
...
def load_plugins(config, plugin_kwargs):
    ...
    Args:
        config (dict): loaded configuration for the Gordon Janitor
        service.
    Returns:
